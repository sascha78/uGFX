...
static bool_t minesGameOver = GFalse;
static bool_t minesGameWinner = GFalse;
...
static bool_t minesFirstGame = GTrue;                  // Just don't clear field for the first time, as we have black screen already... :/
static bool_t minesSplashTxtVisible = GFalse;
...
static void printStats(void)
{
    ...
    gdispFillString(minesStatusIconWidth+8, gdispGetHeight()-11, "    ", font, GFXBLACK, GFXBLACK);
    gdispDrawString(minesStatusIconWidth+8, gdispGetHeight()-11, pps_str, font, GFXWHITE);
    uitoa(minesFlags, pps_str, sizeof(pps_str));
    gdispFillString(8+(minesStatusIconWidth*2)+gdispGetStringWidth("99999", font), gdispGetHeight()-11, "    ", font, GFXBLACK, GFXBLACK);
    gdispDrawString(8+(minesStatusIconWidth*2)+gdispGetStringWidth("99999", font), gdispGetHeight()-11, pps_str, font, GFXWHITE);
    gdispCloseFont(font);
}
...
static void minesUpdateTime(void)
{
    ...
    gdispFillArea((MINES_FIELD_WIDTH*MINES_CELL_WIDTH)-gdispGetStringWidth("9999", font), gdispGetHeight()-15, gdispGetWidth(), 15, GFXBLACK);
    gdispDrawString((MINES_FIELD_WIDTH*MINES_CELL_WIDTH)-gdispGetStringWidth(pps_str, font), gdispGetHeight()-15, pps_str, font, GFXLIME);
    gdispCloseFont(font);
}
...
static void showOne(int16_t x, int16_t y)
{
    ...
    gdispFillArea((x*MINES_CELL_WIDTH)+1, (y*MINES_CELL_HEIGHT)+1, MINES_CELL_WIDTH-1, MINES_CELL_HEIGHT-1, GFXBLACK);
...
static void printGameOver(void)
{
    if (minesGameOver) {
        ...
        if (minesGameWinner) {
            gdispDrawString((gdispGetWidth()-gdispGetStringWidth("You LIVE!", font))/2, gdispGetHeight()-15, "You LIVE!", font, GFXWHITE);
        } else {
            gdispDrawString((gdispGetWidth()-gdispGetStringWidth("You DIED!", font))/2, gdispGetHeight()-15, "You DIED!", font, GFXWHITE);
        }
        gdispCloseFont(font);
    } else {
        gdispFillArea(0, gdispGetHeight()-25, gdispGetWidth(), 25, GFXBLACK);
    }
}
    ...
    if (!minesFirstGame) {
        for (x = 0; x < MINES_FIELD_WIDTH; x++) {
            for (y = 0; y < MINES_FIELD_HEIGHT; y++) {
                ...
                gdispFillArea((i*MINES_CELL_WIDTH)+1, (j*MINES_CELL_HEIGHT)+1, MINES_CELL_WIDTH, MINES_CELL_HEIGHT, GFXBLACK);
