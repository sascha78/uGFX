...
void uGFXMain(void) {
	...
	// Open the connection
	gdispDrawStringBox(0, 0, gdispGetWidth(), gdispGetHeight(), "Connecting to host...", font, GFXWHITE, justifyCenter);
	StartSockets();
	...
	while(getpkt(cmd, 1)) {
		...
		case GNETCODE_BLIT:
			...
			case GNETCODE_SCROLL:
				...
				gdispVerticalScroll(cmd[0], cmd[1], cmd[2], cmd[3], cmd[4], GFXBLACK);
				break;
		#endif
